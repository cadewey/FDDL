...
unsigned int
 fddl_forest::hashnode(const level k, const node_idx p) const
{
    Node *nodeP;
    unsigned int val;
...
int fddl_forest::compare(const level k, const node_idx p, const node_idx q) const
{
    Node *nodeP;
    Node *nodeQ;
...
node_idx fddl_forest::InternalMax(level k, node_idx p, node_idx q)
{
	...
	for (arc_idx i = 0; i < (psize > qsize ? psize : qsize); i++) {
	    node_idx
		u =
		InternalMax(k - 1, i < psize ? FULL_ARC(k, nodeP, i) : 0,
...
void fddl_forest::PrintVals(MDDHandle root, level k)
{
...
void fddl_forest::PrintRanges(MDDHandle root, level * mask)
{
    node_idx newRoot;
    print_node *p;
    print_node *stack;
...
void fddl_forest::PrintAddy(MDDHandle root, level k)
{
    node_idx newRoot;
    level mask[K + 1];
...
void fddl_forest::PrintRanges(level k, node_idx p, level * mask,
...
int fddl_forest::InternalPrintVals(level k, node_idx p)
{
    int flag;
...
void fddl_forest::PrintStates(level k, node_idx p, int *stateArray)
{
	...
	if (nodeP->flags & SPARSE) {
		...
		PrintStates(k - 1, FULL_ARC(k, nodeP, i * 2 + 1),
			    stateArray);
	    }
	} else {
	    for (i = 0; i < nodeP->size; i++) {
...
void fddl_forest::PruneMDD(MDDHandle p)
{
