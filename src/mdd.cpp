...
void Forest::SetArc(level k, node_idx p, arc_idx i, node_idx j)
{
	...
	if (i >= nodeP->size) {	//If we're extending a node (in place)
		...
		for (arc_to_clear = nodeP->size; arc_to_clear < i;
		     arc_to_clear++) {
	...
	for (int i = 0; i < last[k]; i++) {
		...
		if (nodeP->flags & SHARED) {
			for (int j = 0; j < nodeP->size; j++) {
				if (FDDL_NODE(k - 1, FDDL_ARC(k, nodeP, j)).
				    flags & SHARED)
					printf("%d ", j);
			}
		}
	...
	if (mask[k] == 1) {
		...
		for (arc_idx i = 1; i < nodeP->size; i++) {
			...
			if (iVal != lastVal) {
				low[k] = cur_low;
				high[k] = i - 1;
				PrintRanges(k - 1, lastVal, mask, stack, low,
					    high);
				cur_low = i;
				lastVal = iVal;
			}
...
node_idx Forest::ProjectVals(level k, node_idx p, level cutoff)
{
	...
	if (k < cutoff) {
		...
		for (node_idx i = 0; i < nodeP->size; i++) {
			SetArc(k, result, i,
			       ProjectVals(k - 1, FDDL_ARC(k, nodeP, i),
					   cutoff));
		}
		result = CheckIn(k, result);
		ProjectCache[k]->add(result, p);
...
void Forest::PrintStates(level k, node_idx p, int *stateArray)
{
	...
	if (p == 0)
		...
		if (nodeP->flags & SPARSE) {
			...
			if (i == nodeP->size) {
				stateArray[k] = (-1);
				PrintStates(k - 1, FDDL_ARC(k, nodeP, i),
					    stateArray);
			} else {
				for (i = 0; i < nodeP->size; i++) {
					stateArray[k] = i;
